...
export function TeamDialogs({
	...
	const handleCropComplete = async (croppedFile: File) => {
		...
		if (isEditOpen && selectedTeam && onUploadTeamAvatar) {
			...
			const uploadResult = await onUploadTeamAvatar(
				selectedTeam.id,
				croppedFile,
			);
			...
			<Dialog open={isCreateOpen} onOpenChange={onCreateOpenChange}>
				<DialogContent className="sm:max-w-[500px]">
					<DialogHeader>
						...
						<DialogDescription>
							Créez une nouvelle équipe pour organiser votre travail
							collaboratif.
						</DialogDescription>
					</DialogHeader>
												...
												<Avatar className="h-20 w-20 cursor-pointer transition-all group-hover:brightness-75">
													...
													>
														{createForm.watch("name")
															? getTeamInitials(createForm.watch("name"))
															: "EQ"}
													</AvatarFallback>
												</Avatar>
										...
										<div className="flex items-start space-x-4">
											...
											>
												<Avatar className="h-20 w-20 cursor-pointer transition-all group-hover:brightness-75">
													...
													>
														{editForm.watch("name")
															? getTeamInitials(editForm.watch("name"))
															: "EQ"}
													</AvatarFallback>
												</Avatar>
												<div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/20 rounded-full">
											...
											<FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
												<div className="space-y-0.5">
													...
													<div className="text-sm text-muted-foreground">
														Une équipe inactive ne peut pas être assignée à de
														nouveaux projets.
													</div>
												</div>
												<FormControl>
