...
export function useTeamMembers(teamId?: string, onTeamDataChange?: () => void) {
  ...
  const fetchAvailableUsers = useCallback(async (targetTeamId?: string) => {
    if (!targetTeamId && !teamId) return
  ...
  const addTeamMember = async (memberData: AddTeamMemberData) => {
    try {
      ...
      setAvailableUsers(prev => prev.filter(u => u.id !== memberData.user_id))
  ...
  const removeTeamMember = async (memberId: string) => {
    try {
      ...
      if (memberToRemove?.user) {
        setAvailableUsers(prev => [...prev, memberToRemove.user].sort((a, b) => 
  ...
  const changeTeamMemberRole = async (memberId: string, newRole: 'member' | 'lead' | 'admin') => {
    try {
      setError(null)
